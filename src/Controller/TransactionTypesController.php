...
class TransactionTypesController extends AppController
{
    ...
    public function index($accountTypeId=null)
    {
        ...
        if(!empty($_GET['branch_id'])){
            $conditions['TransactionTypes.branch_id'] = $_GET['branch_id'];
        }
        ...
        $branches = $this->TransactionTypes->Branches->find('list');
        $this->set(compact('transactionTypes','accountTypeId','branches'));
        $this->set('_serialize', ['transactionTypes']);
    }

    public function indexLinkedTypes($accountTypeId=null)
    {
        $conditions = [];
        $limit = 100;

        $conditions['TransactionTypes.linked_transaction_type_id IS NOT'] = NULL;
	...
	public function indexNonCommission($accountTypeId=null)
    {
        $conditions = [];
        $limit = 100;
		...
		$conditions['TransactionTypes.commission_structure_id IS'] = NULL;
	...
	public function indexCommission($accountTypeId=null)
    {
        $conditions = [];
        $limit = 100;
		...
		$conditions['TransactionTypes.commission_structure_id IS NOT'] = NULL;
    ...
    public function add($accountTypeId=null)
    {
        ...
        if (!empty($accountTypeId)) {
            $fromAccounts = $this->TransactionTypes->FromAccounts->find('list', ['limit' => 500,'conditions'=>[
                'FromAccounts.id'=>$accountTypeId
            ]]);
            $toAccounts = $this->TransactionTypes->ToAccounts->find('list', ['limit' => 500,'conditions'=>[
                'NOT'=>[
                    'ToAccounts.id'=>$accountTypeId
                ]
            ]]);
        }else{
            $fromAccounts = $toAccounts = $this->TransactionTypes->FromAccounts->find('list', ['limit' => 500]);
        }

        $fromBranches = $toBranches = $branches = $this->TransactionTypes->Branches->find('list', ['limit' => 500]);
        
        $commissionStructures = $this->TransactionTypes->CommissionStructures->find('list', ['limit' => 500]);
        $transactionStatuses = $this->TransactionTypes->TransactionStatuses->find('list', ['limit' => 500]);
        $currencies = $this->TransactionTypes->Currencies->find('list', ['limit' => 500]);
        $accountTypes = $this->TransactionTypes->AccountTypes->find('list', ['limit' => 500]);
        $linkedTransactionTypes = $this->TransactionTypes->LinkedTransactionTypes->find('list', ['limit' => 500]);
        if(!empty($_GET['copy'])){
            $transactionType = $this->TransactionTypes->get($_GET['copy']);
        }
    ...
    public function edit($id = null,$accountTypeId=null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if(!$_break){
                ...
                $old_balance_sheet_side = $transactionType->balance_sheet_side;
                $new_balance_sheet_side = $this->request->data['balance_sheet_side'];

                $old_branch_id = $transactionType->branch_id;
                $new_branch_id = $this->request->data['branch_id'];

                $old_from_account_id = $transactionType->from_account_id;
                $new_from_account_id = $this->request->data['from_account_id'];

                $old_to_account_id = $transactionType->to_account_id;
                $new_to_account_id = $this->request->data['to_account_id'];

                $transactionType = $this->TransactionTypes->patchEntity($transactionType, $this->request->data);
                if ($this->TransactionTypes->save($transactionType)) {

                    // Make Sure the Transactions always match their transaction type currencies, balance_sheet_side
                    if(1 || $oldCurrencyId!=$newCurrencyId || $oldCurrencyId!=$newCurrencyId || $old_branch_id!=$new_branch_id || $old_from_account_id!=$new_from_account_id || $old_to_account_id!=$new_to_account_id){
                        $this->TransactionTypes->Transactions
                        ->query()
                        ->update()
                        ->set([
                            'balance_sheet_side' => $new_balance_sheet_side,
