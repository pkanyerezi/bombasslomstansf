                        ...
                        if (isset($transactionType->custom_fields) && !empty($transactionType->custom_fields)) {
                            ...
                            foreach ($custom_fields as $value) {
                                if (isset($custom_field_options[$value]) && is_array($custom_field_options[$value])) {
                                    ...
                                    if($value=='commission' && !empty($relatedCommissionStructures)){
                                        echo '<div class="row" style="margin-left: -15px;margin-right: -10px;">';
                                            ...
                                            $_options = [];
                                            foreach($relatedCommissionStructures as $cs){
                                                $_options[$cs->id] = 
                                                    $cs->transaction_type->currency_id . 
                                                    ' ('. $cs->name .')';
                                            }
                                            
                                            echo '<div class="col-md-6" title="The option title is the combination of TransactionTypeCurrencyID and commissionStructureName to be used. Make sure that the CommissionStructure has a correct TransactionType that is will use">';
                                                echo $this->Form->input('commission_structure_id',[
                                                    'label'=>'Commission Type',
                                                    'options'=>$_options,
                                                    'value'=>$transactionType->commission_structure->id,
                                                    'class'=>'form-control'
                                                ]);
                                            echo '</div>';
                                        echo '</div>';
