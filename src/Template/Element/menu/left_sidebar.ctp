    ...
    <div class="sidebar">
        ...
        <div class="user-panel">
            ...
            <div class="pull-left info">
                ...
                <a href="<?=$this->request->webroot?>users/logout"><i class="fa fa-power-off"></i> logout user</a>
            </div>
        </div>
        <!-- search form -->
        ...
        <ul class="sidebar-menu">
            ...
            <?php if(count($menuTransactionTypes)):?>
                ...
                <?php $currentBalanceSheetSide = 1;?>
				<?php $currentBalanceSheetSideChanged = false;?>
                ...
                <?php foreach ($menuTransactionTypes as $key=>$transactionType): ?>
					...
					<?php $currentBalanceSheetSideChanged = ($currentBalanceSheetSide==$transactionType->balance_sheet_side)?false:true;?>
                    <?php $currentCurrency = $transactionType->currency_id;?>
					<?php $currentBalanceSheetSide = $transactionType->balance_sheet_side;?>
					...
					<?php if($currentBalanceSheetSideChanged):?>
                        <?php $subTreeOpen = true;?>
                        <li class="treeview">
                            <a href="#">
                                <i class="glyphicon glyphicon-chevron-down"></i>
                                <span>Receieved/Debit</span>
                                <i class="glyphicon glyphicon-angle-left pull-right"></i>
                            </a>
                            <ul class="treeview-menu">
                    <?php endif;?>
            ...
            <li class="treeview <?=(in_array($this->request->params['controller'], ['Transactions','TransactionStatuses','TransactionTypes','CommissionStructures'])?'active':'')?>">
                ...
                <ul class="treeview-menu">
                    ...
                    <li  class="<?=(($this->request->params['controller']== 'TransactionTypes' && $this->request->params['action']=='indexLinkedTypes')?'active':'')?>">
                        <?= $this->AclHtml->link(__('Linked Types'), ['controller' => 'TransactionTypes', 'action' => 'indexLinkedTypes'],['title'=>'Transaction Types that have linked transaction types. Usually used in inter-branch transfers']) ?>
                    </li>
            ...
            <li class="treeview <?=(in_array($this->request->params['controller'], ['Transactions','Accounts'])?'active':'')?>">
                ...
                <ul class="treeview-menu">
                    <li class="<?=(($this->request->params['controller']== 'Dashboards' && $this->request->params['action']=='dailyBalancing')?'active':'')?>">
                        <?= $this->Html->link(__('<i class="fa fa-file"></i><span>Daily Balancing</span>'), ['action' => 'dailyBalancing','controller'=>'Dashboards'],['class' => '','escape'=>false,'data-toggle'=>'tooltip']) ?>
                    </li>
                    ...
                    <li class="<?=(($this->request->params['controller']== 'Transactions' && $this->request->params['action']=='statement')?'active':'')?>">
                        <?= $this->AclHtml->link(__('Cash Flow'), ['controller' => 'Transactions', 'action' => 'statement']) ?>
                    </li>
                    <li class="<?=(($this->request->params['controller']== 'Transactions' && $this->request->params['action']=='balance')?'active':'')?>">
                        <?= $this->AclHtml->link(__('Cash Flow By Transaction Type'), ['controller' => 'Transactions', 'action' => 'balance']) ?>
                    </li>
                </ul>
            </li>
