    ...
    <div class="col-xs-6 col-xs-offset-3">
        <div class="box box-primary">
            ...
            <div class="box-body table-responsive">
                ...
                <fieldset>
                    <legend><?= __('Edit Commission Structure') ?></legend>
                    <?php
                        echo $this->Form->input('name',['class'=>'form-control']);
                        echo $this->Form->input('tag',['class'=>'form-control','title'=>'Unique Tag/Group to relate to similar commissionStructres of other currencies','placeholder'=>'e.g Group1']);
                        echo $this->Form->input('description',['type'=>'text','class'=>'form-control']);
                        // echo $this->Form->input('pricing_structure',['class'=>'form-control']);
                    ...
                    <?php @$pricing_structure = json_decode($commissionStructure->pricing_structure,true); ?>
                    <table class="table my-price-options">
                        ...
                        <?php if(isset($pricing_structure['min_price'])):?>
                            <?php $counter=0;?>
                            <?php foreach($pricing_structure['min_price'] as $val):?>
                            <tr class="po<?=$counter?>">
                                <td>
                                    <input class="form-control" type="text" name="pricing_structure[min_price][]" value="<?=$pricing_structure['min_price'][$counter];?>" />
                                </td>
                                <td>
                                    <input class="form-control" type="text" name="pricing_structure[max_price][]" value="<?=$pricing_structure['max_price'][$counter];?>" />
                                </td>
                                <td>
                                    <input class="form-control" type="text" name="pricing_structure[comm_amount][]" value="<?=$pricing_structure['comm_amount'][$counter];?>" />
                                </td>
                                <td>
                                    <input class="form-control" type="text" name="pricing_structure[comm_perc][]" value="<?=$pricing_structure['comm_perc'][$counter];?>" />
                                </td>
                                <td><span onclick="$('.po<?=$counter?>').remove();" style="cursor:pointer;line-height: 35px;" class="remove-po" id="po<?=$counter?>"><i class="glyphicon glyphicon-trash"></i> </span></td>
                            </tr>
                            <?php $counter++;?>
                            <?php endforeach;?>
                        <?php endif;?>
    ...
    $(document).ready(function(){
        ...
        $('.add-po').click(function(){
            ...
            var body = '<tr class="po'+(counter+2)+'">';
                ...
                body += '<td><span onclick="$(\'.\'+($(this).attr(\'id\'))).remove();" style="cursor:pointer;line-height: 35px;" class="remove-po" id="po'+(counter+2)+'"><i class="glyphicon glyphicon-remove"></i> </span></td>';
                body += '</tr>';
